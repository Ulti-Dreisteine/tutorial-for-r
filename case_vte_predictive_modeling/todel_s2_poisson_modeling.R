...
model <-
  glm(VTE ~ ., data = dat_train, family = poisson(link = "log"))
...
# 训练集
y_pred.train.probs <-
  predict(model, type = "response", newdata = dat_train)
...
# 测试集
y_pred.test.probs <-
  predict(model, type = "response", newdata = dat_test)
...
write.csv(roc_df,
          "case_vte_predictive_modeling/file/roc_curve_poisson.csv",
          row.names = FALSE)
write.csv(pr_df,
          "case_vte_predictive_modeling/file/pr_curve_poisson.csv",
          row.names = FALSE)
