...
# 训练集
y_pred.train.probs <- predict(model, type = "response", newdata = dat_train)
y_pred.train.labels <- ifelse(y_pred.train.probs > 0.5, 1, 0)
y_true.train.labels <- dat_train$VTE

rocr_obj.train <- prediction(y_pred.train.probs, y_true.train.labels)

auc.train <- performance(rocr_obj.train, "auc")@y.values[[1]]
roc.train <- performance(rocr_obj.train, "tpr", "fpr")

plot(roc.train@x.values[[1]], roc.train@y.values[[1]], type = "l", ylim = c(0.0, 1.0), xlab = "TPR", ylab = "FPR")

# 测试集
y_pred.test.probs <- predict(model, type = "response", newdata = dat_test)
...
plot(roc.test@x.values[[1]], roc.test@y.values[[1]], type = "l", ylim = c(0.0, 1.0), xlab = "TPR", ylab = "FPR")
...
plot(pr.test@x.values[[1]], pr.test@y.values[[1]], type = "l", ylim = c(0.0, 1.0), xlab = "Precision", ylab = "Recall")

# ---- 7. 保存结果 -------------------------------------------------------------

roc_df <- data.frame(x = roc.test@x.values[[1]], y = roc.test@y.values[[1]])
pr_df <- data.frame(x = pr.test@x.values[[1]], y = pr.test@y.values[[1]])

write.csv(roc_df, "case_vte_predictive_modeling/file/roc_curve_logr.csv", row.names = FALSE)
write.csv(pr_df, "case_vte_predictive_modeling/file/pr_curve_logr.csv", row.names = FALSE)
